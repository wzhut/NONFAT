...
class FreqEmbedding:
    def __init__(self, cfg):
        ...
        self.tr_T = torch.tensor(cfg['tr_T'], dtype=torch.float32)
        self.tr_y = torch.tensor(cfg['tr_y'], dtype=torch.float32)

        self.te_idx = torch.tensor(cfg['te_idx'], dtype=torch.int64)
        self.te_T = torch.tensor(cfg['te_T'], dtype=torch.float32)
        self.te_y = torch.tensor(cfg['te_y'], dtype=torch.float32)
        ...
        self.x_laggauss, self.w_laggauss, self.x_norm = torch.tensor(x, dtype=torch.float32), torch.tensor(w, dtype=torch.float32), torch.tensor(x_norm, dtype=torch.float32)
        

        self.log_ls_f1 = torch.tensor(np.zeros((self.n_mode, self.dim_embedding_u)), dtype=torch.float32) # K X R
        self.log_ls_f2 = torch.tensor(np.zeros((self.n_mode, self.dim_embedding_u)), dtype=torch.float32) # K X R
        ...
        for k in range(self.n_mode):
            self.Z_f.append(torch.tensor(init_Z[k], dtype=torch.float32))
            self.M_f.append(torch.tensor(np.zeros((self.dim_embedding_u, self.n_pseudo1[k], self.n_laggauss)), dtype=torch.float32))
            self.L_f_row.append(torch.tensor(np.tile(np.eye(self.n_pseudo1[k]), (self.dim_embedding_u, 1)).reshape((self.dim_embedding_u, self.n_pseudo1[k], self.n_pseudo1[k])), dtype=torch.float32))
            self.L_f_col.append(torch.tensor(np.tile(np.eye(self.n_laggauss), (self.dim_embedding_u, 1)).reshape((self.dim_embedding_u, self.n_laggauss, self.n_laggauss)), dtype=torch.float32))
        # self.Z_f = torch.tensor(init_Z, dtype=torch.float32)
        # self.Z_f = torch.tensor(np.random.rand(self.n_mode, self.n_pseudo, self.dim_embedding_v), dtype=torch.float32) # K x Z x s
        # self.M_f = torch.tensor(np.random.randn(self.n_mode, self.dim_embedding_u, self.n_pseudo, self.n_laggauss), dtype=torch.float32) # K x R x Z x C
        # self.M_f = torch.tensor(np.zeros((self.n_mode, self.dim_embedding_u, self.n_pseudo1, self.n_laggauss)), dtype=torch.float32) # K x R x Z x C
        # self.L_f_row = torch.tensor(np.tile(np.eye(self.n_pseudo1), (self.n_mode * self.dim_embedding_u, 1)).reshape((self.n_mode, self.dim_embedding_u, self.n_pseudo1, self.n_pseudo1)), dtype=torch.float32) # K x R x Z x Z
        # self.L_f_col = torch.tensor(np.tile(np.eye(self.n_laggauss), (self.n_mode * self.dim_embedding_u, 1)).reshape(self.n_mode, self.dim_embedding_u, self.n_laggauss, self.n_laggauss), dtype=torch.float32) # K x R x C x C

        self.Z_g = torch.tensor(np.random.randn(self.n_pseudo2, self.dim_embedding_u * self.n_mode), dtype=torch.float32) # Z x KR
        self.m_g = torch.tensor(np.zeros((self.n_pseudo2, 1)), dtype=torch.float32)
        # self.m_g = torch.tensor(np.random.randn(self.n_pseudo, 1), dtype=torch.float32) # Z x 1
        self.L_g = torch.tensor(np.eye(self.n_pseudo2), dtype=torch.float32) # Z x Z

        self.log_ls_g = torch.tensor([0.], dtype=torch.float32)
        # self.log_ls_g = torch.tensor([np.log(self.n_mode * self.dim_embedding_u)], dtype=torch.float32)
        self.log_tau = torch.tensor([0.], dtype=torch.float32)

        # node identity embeddings
        self.v = []
        for i in range(self.n_mode):
            self.v.append(torch.tensor(init_v[i], dtype=torch.float32))
            # self.v.append(torch.tensor(np.random.rand(self.n_node[i], self.dim_embedding_v), dtype=torch.float32))
    ...
    def get_nELBO(self, batch_idx, batch_T, batch_y):
        ...
        for k in range(self.n_mode):
            ...
            # Sigma = Sigma + 1e-5 * torch.eye(self.n_laggauss, dtype=torch.float32, device=Sigma.device).view((1, 1, self.n_laggauss, self.n_laggauss))
