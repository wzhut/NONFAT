...
def test_beijing5(rank=3, save=False):
    ...
    mae_list = []
    for fold in range(5):
        train_ind = data['train_folds'][fold][:, :2]
        train_time = data['train_folds'][fold][:, 2:3]
        train_y = data['train_folds'][fold][:, 3:]
        test_ind = data['test_folds'][fold][:, :2]
        test_time = data['test_folds'][fold][:, 2:3]
        test_y = data['test_folds'][fold][:, 3:]
    ...
    if save:
        np.save('GPTF_beijing_5fold_rank_{}_res.npy'.format(rank), res)
...
def test_server5(rank=3, save=False):
    data_file = '../data/server/server_100k.npy'
    data = np.load(data_file, allow_pickle=True).item()
    ndims = data['ndims']
    data = data['data']
    res = []
    ...
    for fold in range(5):
        ...
        mae_list.append(np.min(te_mae_list))
    if save:
        np.save('GPTF_server_5fold_rank_{}_res.npy'.format(rank), res)
    with open('GPTF_time.txt', 'a') as f:
...
def test_traffic5(rank=3, save=False):
    ...
    for fold in range(5):
        ...
        model = GPTF(train_ind, train_y, train_time, U, m, batch_size, torch.device('cpu'), test_every=10)
        tr_rmse_list, tr_mae_list, tr_ll_list, tr_pred_m_list, tr_pred_std_list, te_rmse_list, te_mae_list, te_ll_list, te_pred_m_list, te_pred_std_list = model.train(test_ind, test_y, test_time, lr, nepoch, '')    
        res.append({'tr_rmse_list':tr_rmse_list, 
                    'tr_ll_list': tr_ll_list, 
                    'tr_pred_m_list': tr_pred_m_list, 
        ...
        mae_list.append(np.min(te_mae_list))
    if save:
        np.save('GPTF_traffic_5fold_rank_{}_res.npy'.format(rank), res)
    with open('GPTF_time.txt', 'a') as f:
        f.write('GPTF_traffic_5fold_rank_{}:\n \
                 rmse: {}\t{}\n\
                 mae: {}\t{}\n\
                 ll: {}\t{}\n'.format(rank, np.mean(rmse_list), np.std(rmse_list)/np.sqrt(5), np.mean(mae_list), np.std(mae_list)/np.sqrt(5), np.mean(ll_list), np.std(ll_list)/np.sqrt(5)))
...
def test_ctr5(rank=3, save=False):
    ...
    for fold in range(5):
        ...
        mae_list.append(np.min(te_mae_list))
    if save:
        np.save('GPTF_ctr_5fold_rank_{}_res.npy'.format(rank), res)
    with open('GPTF_time.txt', 'a') as f:
...
def test_dblp5(rank=3, save=False):
    ...
    for fold in range(5):
        train_ind = data[fold]['tr_ind']
        train_time = data[fold]['tr_T']
        train_y = data[fold]['tr_y'].astype(np.float32)
        test_ind = data[fold]['te_ind']
        test_time = data[fold]['te_T']
        test_y = data[fold]['te_y'].astype(np.float32)
        ...
        mae_list.append(np.min(te_mae_list))
    if save:
        np.save('GPTF_dblp_5fold_rank_{}_res.npy'.format(rank), res)
    with open('GPTF_time.txt', 'a') as f:
        f.write('GPTF_dblp_5fold_rank_{}:\n \
                 rmse: {}\t{}\n\
                 mae: {}\t{}\n\
                 ll: {}\t{}\n'.format(rank, np.mean(rmse_list), np.std(rmse_list)/np.sqrt(5), np.mean(mae_list), np.std(mae_list)/np.sqrt(5), np.mean(ll_list), np.std(ll_list)/np.sqrt(5)))

def test_server_extra(rank=3, save=False):
    data_file = '../data/server/server_40k_extra.npy'
    data = np.load(data_file, allow_pickle=True).item()
    ndims = data['ndims']
    data = data['data']
    res = []
    ...
    for fold in range(1):
        ...
        mae_list.append(np.min(te_mae_list))
    if save:
        np.save('GPTF_server_extra_rank_{}_res.npy'.format(rank), res)
    with open('GPTF_time.txt', 'a') as f:
        f.write('GPTF_server_extra_rank_{}:\n \
                 rmse: {}\t{}\n\
                 mae: {}\t{}\n\
                 ll: {}\t{}\n'.format(rank, np.mean(rmse_list), np.std(rmse_list)/np.sqrt(5), np.mean(mae_list), np.std(mae_list)/np.sqrt(5), np.mean(ll_list), np.std(ll_list)/np.sqrt(5)))

def test_beijing_extra(rank=3, save=False):
    data_file = '../data/Beijing/beijing_extrap.npy'
    data = np.load(data_file, allow_pickle=True).item()
    ndims = data['nvec']
    res = []
    rmse_list = []
    ...
    for fold in range(1):
        ...
        mae_list.append(np.min(te_mae_list))
    # if save:
    #     np.save('GPTF_beijing_extra_rank_{}_res.npy'.format(rank), res)

    if save:
            time = np.linspace(0, 10, num=100)
            for i in range(ndims[0]):
                for j in range(ndims[1]):
                    batch_idx = np.tile([i, j], [100, 1])
                    batch_T = time.reshape((-1, 1))
                    pred_m, pred_std = model.pred_np(batch_idx, batch_T)
                    
                    tr_sel = (train_ind[:, 0] == i) & (train_ind[:, 1] == j)
                    tr_tp = train_time[tr_sel]
                    tr_yp = train_y[tr_sel]

                    te_sel = (test_ind[:, 0] == i) & (test_ind[:, 1] == j)
                    te_tp = test_time[te_sel]
                    te_yp = test_y[te_sel]

                    savemat('i_{}_j_{}.mat'.format(i, j), {
                        'pred_m': pred_m,
                        'pred_std': pred_std,
                        'tr_tp': tr_tp,
                        'tr_yp': tr_yp,
                        'te_tp': te_tp,
                        'te_yp': te_yp,
                        'min_te_time': np.min(test_time)
                    })        
    with open('GPTF_time.txt', 'a') as f:
        f.write('GPTF_beijing_extra_rank_{}:\n \
                 rmse: {}\t{}\n\
                 mae: {}\t{}\n\
                 ll: {}\t{}\n'.format(rank, np.mean(rmse_list), np.std(rmse_list)/np.sqrt(5), np.mean(mae_list), np.std(mae_list)/np.sqrt(5), np.mean(ll_list), np.std(ll_list)/np.sqrt(5)))

def test_beijing(rank=3, save=False):
    data_file = '../data/Beijing/beijing.npy'
    data = np.load(data_file, allow_pickle=True).item()
    ndims = data['nvec']
    res = []
    ...
    if save:
            time = np.linspace(0, 15, num=150)
            for i in range(ndims[0]):
                for j in range(ndims[1]):
                    batch_idx = np.tile([i, j], [150, 1])
                    batch_T = time.reshape((-1, 1))
                    pred_m, pred_std = model.pred_np(batch_idx, batch_T)
...
if __name__ == '__main__':
    test_beijing(3, True)
    # test_beijing_extra(3, True)
    # test_beijing_extra(2)
    # test_beijing_extra(5)
    # test_beijing_extra(7)
    # test_beijing5(3)
    # test_beijing5(5)
    # test_beijing5(7)
