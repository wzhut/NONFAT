...
def test_beijing5(rank=3):
    ...
    rmse_list = []
    mae_list = []
    time = np.concatenate([data['train_folds'][0][:, 2:3], data['test_folds'][0][:, 2:3]], axis=0)
    ...
    for fold in range(5):
        ...
        model = Neural_rnn(len(ndims), ndims, R, 51, batch_size)
        model.todev(torch.device('cpu'))
        tr_rmse_list, tr_mae_list, te_rmse_list, te_mae_list = model.train(train_dataset, test_dataset, nepoch, lr)
        rmse_list.append(np.min(te_rmse_list))
        mae_list.append(np.min(te_mae_list))
    with open('Neural_rnn.txt', 'a') as f:
